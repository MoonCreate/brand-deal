    ...
    contract BrandNFT is ERC721URIStorage, Ownable {
        ...
        event BrandNFTMinted(uint256 indexed tokenId, address indexed owner, string indexed instanceName, string metadataURI);
        ...
        function mintBrandNFT(address to, string calldata _instanceName, string calldata _metadataURI, uint256 _NIB)
            external
            returns (uint256)
        {
            if(to == address(0)){
