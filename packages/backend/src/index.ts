...
ponder.on("BrandDeal:CreatorApply", async ({ event, context }) => {
  const { campaignId, creatorAddress } = event.args;
  const id = `${campaignId}-${creatorAddress}`;
...
ponder.on("BrandDeal:CreatorCancelledApplyForCampaign", async ({ event, context }) => {
