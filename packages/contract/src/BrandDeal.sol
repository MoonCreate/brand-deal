...
contract BrandDeal is Ownable {
    ...
    event CreatorCancelledApplyForCampaign(
        uint256 indexed campaignId,
    ...
    modifier onlyRegisteredBrand() {
        require(
            brandNFT.balanceOf(_msgSender()) > 0,
            "Caller is not a registered brand (no Brand NFT)"
        );
        _;
    }
    ...
    modifier onlyRegisteredCreator() {
        require(
            creatorNFT.balanceOf(_msgSender()) > 0,
            "Caller is not a registered creator (no Creator NFT)"
        );
        _;
    }
    ...
    modifier appliedCampaign(uint256 _campaignId) {
        require(
            creatorPool[_campaignId][msg.sender] == true,
            "The caller has already been applied to this Campaign"
        );
        _;
    }

    modifier onlyBrandOfCampaign(uint256 _campaignId) {
        (, address ownerCampaign, , , , , , ) = campaignContract.campaigns(
            _campaignId
        );
        require(
            ownerCampaign == _msgSender(),
            "Caller is not the brand of this campaign"
        );
        _;
    }

    modifier onlyCampaignCreator(uint256 _campaignId) {
        (, , address approvedCreator, , , , , ) = campaignContract.campaigns(
            _campaignId
        );
        require(
            approvedCreator == _msgSender(),
            "Caller is not the creator of this campaign"
        );
        _;
    }
    ...
    modifier onlyAdminOrBrandOfCampaign(uint256 _campaignId) {
        ...
        require(
            owner() == _msgSender() || ownerCampaign == _msgSender(),
            "Caller is not admin or brand"
        );
        _;
    }
    ...
    function registerBrand(
        ...
        uint256 brandNFTId = brandNFT.mintBrandNFT(
            _msgSender(),
            _instanceName,
            _metadataURI,
            _NIB
    ...
    function registerCreator(
        ...
        if (creatorNFT.balanceOf(_msgSender()) > 0) {
            revert(
                "The owner has already registered a creator (no Creator NFT)"
            );
        }
        ...
        uint256 creatorNFTId = creatorNFT.mintCreatorNFT(
            _msgSender(),
            _name,
            _metadataURI
        );
    ...
    function createCampaign(
        ...
        uint256 campaignNFTId = campaignContract.mintCampaignNFT(
            _msgSender(),
            _campaignName,
            _rewards,
            _deadline,
    ...
    function creatorApplyToCampaign(
        uint256 _campaignId
    ) public onlyRegisteredCreator {
    ...
    function assignCreatorToCampaign(
        uint256 _campaignId,
        address _creatorAddress
    ) public onlyBrandOfCampaign(_campaignId) {
    ...
    function submitTaskCampaignCreator(
        uint256 _campaignId,
        string memory _submitMetadataUri
    ) public onlyCampaignCreator(_campaignId) {
        campaignContract.submitTaskCampaignCreator(
            _campaignId,
            _submitMetadataUri
        );
    ...
    function cancelApply(
        uint256 _campaignId
    ) public appliedCampaign(_campaignId) {
    ...
    function resolveCampaign(
        uint256 _campaignId
    ) public onlyAdminOrBrandOfCampaign(_campaignId) {
        (
            ,
            ...
            uint256 campaignDeadline,
            ,
            string memory submitMetadataUri
        ) = campaignContract.campaigns(_campaignId);
        if (approvedCreator == address(0))
            revert("The creator is not approved for this Campaign yet.");
        if (bytes(submitMetadataUri).length == 0)
            revert("The creator not have been submitted the campaign");
        if (block.timestamp <= campaignDeadline)
            revert("Cannot resolve the campaign until deadline");
